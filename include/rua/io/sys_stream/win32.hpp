...
public:
	...
	virtual ptrdiff_t read(bytes_ref p) {
		...
		if (!sch->is_own_stack()) {
			...
			if (buf) {
				auto sz = rua::wait(std::move(sch), _read, _h, bytes_ref(buf));
				if (sz > 0) {
					p.copy_from(buf);
				}
				return sz;
		...
		return rua::wait(std::move(sch), _read, _h, p);
	}
	...
	virtual ptrdiff_t write(bytes_view p) {
		...
		if (!sch->is_own_stack()) {
			...
			if (data) {
				return rua::wait(std::move(sch), _write, _h, bytes_view(data));
			}
		}
		return rua::wait(std::move(sch), _write, _h, p);
	}
