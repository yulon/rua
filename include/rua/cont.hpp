#ifndef _RUA_CONT_HPP
#define _RUA_CONT_HPP

#include "macros.hpp"

#if !defined(RUA_AMD64) && !defined(RUA_I386)
	#error rua::cont: not supported this platform!
#endif
	...
	*/
	...
	class cont {
		public:
			...
			cont(void (*func)(any_word), any_word func_param, bin_ref stack) {
			...
			bool save() {
				static const uint8_t code[]{
							...
							#include "cont/save_amd64_win.inc"
						#else
							#include "cont/save_amd64_sysv.inc"
						#endif
					#elif defined(RUA_I386)
						#ifdef _WIN32
							#include "cont/save_i386_win.inc"
						#else
							#include "cont/save_i386.inc"
						#endif
					#endif
				};

				static bool (*fn)(cont *) = _code_init(code);
			...
			void restore() const {
				static const uint8_t code[]{
					#ifdef RUA_AMD64
						#ifdef RUA_MS64_FASTCALL
							#include "cont/restore_amd64_win.inc"
						#else
							#include "cont/restore_amd64_sysv.inc"
						#endif
					#elif defined(RUA_I386)
						#ifdef _WIN32
							#include "cont/restore_i386_win.inc"
						#else
							#include "cont/restore_i386.inc"
						#endif
					#endif
				};

				static void (*fn)(const cont *) = _code_init(code);
			...
			/*
				// For reference only.
				void restore(cont &cur_cont_saver) const {
			...
			*/
			...
			void restore(cont &cur_cont_saver) const {
				static const uint8_t code[]{
					#ifdef RUA_AMD64
						#ifdef RUA_MS64_FASTCALL
							#include "cont/restore_and_save_amd64_win.inc"
						#else
							#include "cont/restore_and_save_amd64_sysv.inc"
						#endif
					#elif defined(RUA_I386)
						#ifdef _WIN32
							#include "cont/restore_and_save_i386_win.inc"
						#else
							#include "cont/restore_and_save_i386.inc"
						#endif
					#endif
				};

				static void (*fn)(const cont *, cont *) = _code_init(code);
	...
	#ifdef _WIN32
		#ifdef RUA_AMD64
			RUA_STATIC_ASSERT(offsetof(cont, osctx) == 152);
		#endif

		#ifdef RUA_I386
			RUA_STATIC_ASSERT(offsetof(cont, osctx) == 48);
		#endif
	#endif
}
