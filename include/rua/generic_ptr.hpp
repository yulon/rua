...
public:
	...
	RUA_FORCE_INLINE constexpr uintptr_t value() const {
		return _val;
	}
	...
	template <typename T>
	RUA_FORCE_INLINE generic_ptr operator+(T &&target) const {
	...
	template <typename T>
	RUA_FORCE_INLINE generic_ptr &operator+=(T &&target) {
