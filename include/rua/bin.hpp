...
public:
	...
	any_ptr derel(ptrdiff_t pos = 0) const {
		return _this()->base() + pos + SlotSize +
			   _this()->template get<RelPtr>(pos);
	}

	template <typename RelPtr, size_t SlotSize = sizeof(RelPtr)>
	RelPtr enrel(ptrdiff_t pos, any_ptr abs_ptr) {
		auto rel_ptr =
			static_cast<RelPtr>(abs_ptr - (_this()->base() + pos + SlotSize));
...
public:
	...
	constexpr bin_view(const void *ptr, size_t sz = nmax<size_t>()) :
		_base(sz ? ptr : nullptr),
		_sz(ptr ? sz : 0) {}

	template <
		typename T,
		typename = typename std::enable_if<
			!std::is_same<typename std::decay<T>::type, void>::value>::type>
	constexpr bin_view(const T *ptr, size_t sz = sizeof(T)) :
		bin_view(reinterpret_cast<const void *>(ptr), sz) {}
	...
	template <
		...
		typename = typename std::enable_if<
			...
			type>
	constexpr bin_view(const T &ref, size_t sz = sizeof(T)) :
		bin_view(&ref, sz) {}

	RUA_CONSTEXPR_14 bin_view(const char *c_str) :
		_base(c_str),
		_sz(strlen(c_str)) {}
...
#ifdef __cpp_lib_string_view
	constexpr bin_view(std::string_view sv) :
		_base(sv.data()),
		_sz(sv.length()) {}
#endif
	...
	template <typename D>
	RUA_FORCE_INLINE D get(ptrdiff_t offset = 0) const {
		return bit_get<D>(_base.to<const void *>(), offset);
	}

	template <typename D>
	RUA_FORCE_INLINE D aligned_get(ptrdiff_t ix = 0) const {
		return bit_aligned_get<D>(_base.to<const void *>(), ix);
	}

	RUA_FORCE_INLINE const uint8_t &operator[](ptrdiff_t offset) const {
		return _base.to<const uint8_t *>()[offset];
	}
...
public:
	...
	constexpr bin_ref(void *ptr, size_t sz = nmax<size_t>()) :
		bin_view(ptr, sz) {}
	...
	template <
		...
		typename = typename std::enable_if<
			...
			!std::is_same<typename std::decay<T>::type, void>::value>::type>
	constexpr bin_ref(T *ptr, size_t sz = sizeof(T)) :
		bin_ref(reinterpret_cast<void *>(ptr), sz) {}
	...
	RUA_FORCE_INLINE uint8_t &operator[](ptrdiff_t offset) {
		return _base.to<uint8_t *>()[offset];
	}
...
public:
	...
	template <
		...
		typename = typename std::enable_if<
			!std::is_same<typename std::decay<T>::type, void>::value>::type>
	bin(const T *ptr, size_t sz = sizeof(T)) :
		bin(bin_view(reinterpret_cast<const void *>(ptr), sz)) {}
	...
	bin(bin &&src) :
		bin_ref(static_cast<bin_ref &&>(std::move(src))),
	...
	bin &slice_self(ptrdiff_t begin, ptrdiff_t end) & {
		...
		assert(
			size() <=
			_alloced_size() - static_cast<size_t>(_base - _alloced_base()));
	...
	bool resize(size_t sz) {
		...
		if (_alloced) {
			if (sz < _alloced_size() -
						 static_cast<size_t>(_base - _alloced_base())) {
				bin_view::resize(sz);
				return false;
			}
